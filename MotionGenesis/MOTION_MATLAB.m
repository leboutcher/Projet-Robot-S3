function [t,VAR,Output] = MOTION_MATLAB
%===========================================================================
% File: MOTION_MATLAB.m created Jun 26 2024 by MotionGenesis 6.3.
% Portions copyright (c) 2009-2023 Motion Genesis LLC.  Rights reserved.
% MotionGenesis Student Licensee: Elisabeth. (until February 2028).
...
Ffb=0; Fff=0; Nb=0; Nf=0; qDDt=0; qWBDDt=0; qWFDDt=0; xDDt=0; Torque=0; Bx=0; By=0; Ccorner1x=0; Ccorner2x=0; Ccorner3x=0; Ccorner4x=0;
WdContactx=0; WdContacty=0; WgContactx=0; WgContacty=0;


%-------------------------------+--------------------------+-------------------+-----------------
% Quantity                      | Value                    | Units             | Description
%-------------------------------|--------------------------|-------------------|-----------------
b                               =  0.03;                   % UNITS               Constant
...
COEF = zeros( 8, 8 );
COEF(1,1) = 0.5*L*(mP+2*mQ)*cos(q);
COEF(1,2) = 1;
COEF(1,3) = 1;
COEF(1,6) = mB + mF + mP + mQ + mR;
COEF(2,1) = 0.5*L*(mP+2*mQ)*sin(q);
COEF(2,4) = -1;
COEF(2,5) = -1;
COEF(3,1) = 0.25*L*(1.333333333333333*L*mP+4*L*mQ+LR*mP*sin(q)+2*LR*mQ*sin(q)-4*mQ*rayon*cos(q)-2*mP*rayon*cos(q));
COEF(3,5) = -LR;
COEF(3,6) = L*mQ*cos(q) + 0.5*L*mP*cos(q) - mB*rayon - mF*rayon - mP*rayon - mQ*rayon - mR*rayon;
COEF(3,7) = 0.5*mB*rayon^2;
COEF(3,8) = 0.5*mF*rayon^2;
COEF(4,1) = 0.3333333333333333*L^2*(mP+3*mQ);
COEF(4,6) = 0.5*L*(mP+2*mQ)*cos(q);
COEF(5,2) = rayon;
...
COEF(8,8) = rayon;
RHS = zeros( 1, 8 );
RHS(1) = 0.5*L*(mP+2*mQ)*sin(q)*qDt^2;
RHS(2) = -g*mB - g*mF - g*mP - g*mQ - g*mR - 0.5*L*(mP+2*mQ)*cos(q)*qDt^2;
RHS(3) = -g*LR*mF - 0.5*g*LR*mP - 0.5*g*LR*mQ - 0.5*g*LR*mR - Torque - g*L*mQ*sin(q) - 0.5*g*L*mP*sin(q) - L*mQ*rayon*sin(q)*qDt^2  ...
- 0.5*L*LR*mQ*cos(q)*qDt^2 - 0.5*L*mP*rayon*sin(q)*qDt^2 - 0.25*L*LR*mP*cos(q)*qDt^2;
RHS(4) = -0.5*g*L*(mP+2*mQ)*sin(q) - b*qDt;
RHS(5) = -Torque;
...
if( isempty(Output) ),
   if( ~isempty(FileIdentifier) ),
      ...
      fprintf( 1, '\n Output is in the files MOTION_MATLAB.i  (i=1,2)\n' );
      ...
      fprintf( 1, '    someName = load( ''MOTION_MATLAB.1'' );\n' );
...
if( isempty(hasHeaderInformationBeenWritten) ),
   if( shouldPrintToScreen ),
      fprintf( 1,                '%%       t              x             x''              q             q''            qWB''           qWF''            Ffb            Fff            Nb             Nf           Torque\n' );
      fprintf( 1,                '%%     (sec)           (m)          (m/sec)         (deg)         (deg/s)       (deg/sec)      (deg/sec)         (N)            (N)            (N)            (N)           (N*m)\n\n' );
   end
   if( shouldPrintToFile && isempty(FileIdentifier) ),
      FileIdentifier = zeros( 1, 2 );
      FileIdentifier(1) = fopen('MOTION_MATLAB.1', 'wt');   if( FileIdentifier(1) == -1 ), error('Error: unable to open file MOTION_MATLAB.1'); end
      fprintf(FileIdentifier(1), '%% FILE: MOTION_MATLAB.1\n%%\n' );
      fprintf(FileIdentifier(1), '%%       t              x             x''              q             q''            qWB''           qWF''            Ffb            Fff            Nb             Nf           Torque\n' );
      fprintf(FileIdentifier(1), '%%     (sec)           (m)          (m/sec)         (deg)         (deg/s)       (deg/sec)      (deg/sec)         (N)            (N)            (N)            (N)           (N*m)\n\n' );
      FileIdentifier(2) = fopen('MOTION_MATLAB.2', 'wt');   if( FileIdentifier(2) == -1 ), error('Error: unable to open file MOTION_MATLAB.2'); end
      fprintf(FileIdentifier(2), '%% FILE: MOTION_MATLAB.2\n%%\n' );
...
%======================================
end    % End of function MOTION_MATLAB
%======================================
